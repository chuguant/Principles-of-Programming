...
# h = tree height of the branch, changed to height() + 1 = log(size() + 1, 2) for this program
        ...
        if not(self.left_node.size() != 0 or self.right_node.size() != 0): # if both nodes are empty, insert into left node
            ...
            if not(self.left_node.height() + 1 != log(self.left_node.size() + 1, 2) \
               or (self.right_node.height() + 1) != log(self.right_node.size() + 1, 2) \
               or self.left_node.size() != self.right_node.size()):
    ...
    def _bubble_up(self, parent = None):
        if not(parent and self.value <= parent.value):
            return True
