...
def get_nodes(tree):
    if tree.value == None:
        return []
    nodes = get_nodes(tree.left_node)
    nodes.extend(get_nodes(tree.right_node))
    nodes.append(tree)
    return nodes
...
##    for node in get_nodes(tree):
##        if node.left_node.value is not None and node.right_node.value is None:
##            left_leave.append(node.value)
##    for node in get_nodes(tree):
##        if node.right_node.value is not None and node.left_node.value is None:
##            right_leave.append(node.value)
##    for node in get_nodes(tree):
##        if node.right_node.value is not None and node.left_node.value is not None:
##            not_leave.append(node.value)
##    print('leaves',leaves)
    if len(leaves) == 1:
        return 0
    return max(leaves[i + 1]- leaves[i] for i in range(len(leaves) - 1))
##    print('left_leave',left_leave)
##    print('right_leave',right_leave)
##    print('not_leave',not_leave)
